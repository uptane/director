...
object TagId {
  ...
  def from(s: String): Either[ValidationError, TagId] =
    if (s.length <= 20 && s.matches("[\\w\\-_ ]+"))
      ...
      Left(
        ValidationError(
          s"$s should contain between one and a twenty alphanumeric, hyphen, underscore or space characters."
        )
      )

  implicit val tagIdCodec: Codec[TagId] =
    Codec.from(validatedGenericDecoder, validatedGenericEncoder)
