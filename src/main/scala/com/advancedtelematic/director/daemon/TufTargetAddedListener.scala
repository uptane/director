...
class TufTargetAddedListener()(implicit val db: Database, val ec: ExecutionContext)
  ...
  private def ecuTargetFromTufTarget(msg: TufTargetAdded): Future[EcuTarget] = {
    ...
    val ecuTarget = EcuTarget(
      ...
      msg.checksum,
      msg.checksum.hash,
      msg.custom.flatMap(_.uri.map(u => Uri(u.toString))),
  ...
  private def filterDevicesWithExistingAssignments(
    assignments: List[Assignment]): Future[List[Assignment]] = async {
    val assignmentExists = await(
      assignmentsRepository.existsForDevices(assignments.map(_.deviceId).toSet)
    ).filter(_._2).keys.toSet

    assignmentExists.foreach { d =>
      _log.info(
        s"Not creating auto update assignment for $d, assignment for that device already exists"
      )
    }
  ...
  private def createAssignments(ns: Namespace,
                                ecuTarget: EcuTarget,
                                autoUpdateDefinitions: List[AutoUpdateDefinition]): Future[Unit] =
    async {
      ...
      val assignments = autoUpdateDefinitions.map { autoUpdate =>
        Assignment(
          ...
          autoUpdate.ecuId,
          ecuTarget.id,
          AutoUpdateId(autoUpdate.id.uuid),
          inFlight = false,
      ...
      newAssignments.foreach { assignment =>
        _log.info(
          s"Creating auto update assignment for ${assignment.deviceId} (target = ${ecuTarget.filename})"
        )
      }

      await(
        assignmentsRepository.persistManyForEcuTarget(
          ecuTargetsRepository,
          provisionedDeviceRepository
        )(ecuTarget, newAssignments)
      )
    }
  ...
  override def apply(msg: TufTargetAdded): Future[_] = async {
    msg.custom.map(_.name) match {
      case Some(name) =>
        val updateDefinitions =
          await(autoUpdateDefinitionRepository.findByName(msg.namespace, name)).toList
