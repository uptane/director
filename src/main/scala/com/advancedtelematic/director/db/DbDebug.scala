...
class DirectorDbDebug()(implicit val db: Database, ec: ExecutionContext) {
  ...
  lazy val namespace_resources = ResourceGroup[Namespace]("namespaces", "namespace", List(
    TableResource("devices",
      readTable("devices", ns => sql"""select * from devices where namespace = $ns order by created_at DESC LIMIT #$DEFAULT_LIMIT""")),
    TableResource("admin_roles",
      readTable("admin_roles", ns => sql"""select a.* from admin_roles a join repo_namespaces r using(repo_id) where r.namespace = $ns  order by `role` ASC, version desc""")),
    TableResource("hardware_updates",
      readTable("hardware_updates", ns => sql"""select * from hardware_updates where namespace = $ns order by created_at ASC LIMIT #$DEFAULT_LIMIT""")),
    TableResource("ecu_targets",
  ...
  lazy val device_resources = ResourceGroup[DeviceId]("devices", "device-id", List(
    ...
    TableResource("device_roles",
      readTable("device_roles", id => sql"""select * from device_roles where device_id = $id order by `role` ASC, version desc""")),
    TableResource("admin_roles",
      readTable("admin_roles", id => sql"""select a.* from admin_roles a join repo_namespaces r using(repo_id) join devices d using (namespace) where d.id = $id  order by `role` ASC, version desc""")),
    TableResource("assignments",
      readTable("assignments", id =>
