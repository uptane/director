...
object SlickMapping {
  ...
  implicit val hashMethodColumn: BaseColumnType[HashMethod] = MappedColumnType.base[HashMethod, String](_.toString, HashMethod.withName)
  implicit val targetFormatMapper: BaseColumnType[TargetFormat.Value] = SlickEnumMapper.enumMapper(TargetFormat)

  implicit val targetUpdateMapper: BaseColumnType[TargetUpdate] = SlickCirceMapper.circeMapper[TargetUpdate]
  ...
  implicit val adminRoleNameMapper: BaseColumnType[AdminRoleName] = validatedStringMapper[AdminRoleName]

  implicit val scheduledUpdatesMapper: BaseColumnType[ScheduledUpdate.Status] = enumeratumMapper(ScheduledUpdate.Status)
