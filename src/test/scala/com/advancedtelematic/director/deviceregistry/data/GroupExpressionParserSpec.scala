...
class GroupExpressionParserSpec extends AnyFunSuite with Matchers {
  ...
  test("parses multiple or expressions with parens") {
    runParser(
      "(deviceid contains bananas1) or deviceid contains oranges or deviceid contains melons"
    ) shouldBe
      Or(
        NonEmptyList
          .of(DeviceIdContains("bananas1"), DeviceIdContains("oranges"), DeviceIdContains("melons"))
      )
  }

  test("parses multiple or expressions with nested parens") {
    runParser(
      "(deviceid contains bananas) or ((deviceid contains oranges) or deviceid contains melons)"
    ) shouldBe
      Or(
        NonEmptyList.of(
          DeviceIdContains("bananas"),
  ...
  test("parses multiple or expressions") {
    runParser(
      "deviceid contains something0 or deviceid contains other0 or deviceid contains other1"
    ) shouldBe
      Or(
        NonEmptyList.of(
          DeviceIdContains("something0"),
          DeviceIdContains("other0"),
          DeviceIdContains("other1")
        )
      )
  }
  ...
  test("parses and/or expression") {
    runParser(
      "deviceid contains bananas and (deviceid contains oranges or deviceid contains melons)"
    )
    And(
      NonEmptyList.of(
        DeviceIdContains("bananas"),
  ...
  test("parses and/or expression without parens") {
    runParser(
      ...
      Or(
        NonEmptyList.of(
          And(NonEmptyList.of(DeviceIdContains("bananas"), DeviceIdContains("oranges"))),
  ...
  test("parses nested expressions when using parens") {
    runParser(
      "(deviceid contains something0) and ((deviceid contains other0) or (deviceid contains melons))"
    ) shouldBe
      And(
        NonEmptyList.of(
          DeviceIdContains("something0"),
  ...
  test("parses boolean expressions without parenthesis") {
    runParser(
      "deviceid contains eo7zOnogw or deviceid contains Ku05MCxEE6GQ2iKh and deviceid contains ySqlJlu"
    ) shouldBe
      Or(
        NonEmptyList.of(
          DeviceIdContains("eo7zOnogw"),
          And(NonEmptyList.of(DeviceIdContains("Ku05MCxEE6GQ2iKh"), DeviceIdContains("ySqlJlu")))
        )
      )
  }
  ...
  test("parses 'deviceid position is' with multiple or expressions") {
    runParser(
      "deviceid position(1) is a or (deviceid position(2) is 8 or deviceid position(3) is A)"
    ) shouldBe
      Or(
        NonEmptyList.of(
          DeviceIdCharAt('a', 0),
          Or(NonEmptyList.of(DeviceIdCharAt('8', 1), DeviceIdCharAt('A', 2)))
        )
      )
  }
  ...
  test("parses 'deviceid position is' with multiple and expressions") {
    runParser(
      "deviceid position(1) is a and (deviceid position(2) is 8 and deviceid position(3) is A)"
    ) shouldBe
      And(
        NonEmptyList.of(
          DeviceIdCharAt('a', 0),
          And(NonEmptyList.of(DeviceIdCharAt('8', 1), DeviceIdCharAt('A', 2)))
        )
      )
  }
  ...
  test("parses 'deviceid position is' or nested 'deviceid contains'") {
    runParser(
      "deviceid position(3) is x or (deviceid contains something0 and deviceid contains something0else)"
    ) shouldBe
      Or(
        NonEmptyList.of(
          DeviceIdCharAt('x', 2),
  ...
  test("parses 'deviceid contains' and nested 'deviceid position is'") {
    runParser(
      "deviceid contains something0 and (deviceid position(1) is x or deviceid position(2) is y)"
    ) shouldBe
      And(
        NonEmptyList.of(
          DeviceIdContains("something0"),
          Or(NonEmptyList.of(DeviceIdCharAt('x', 0), DeviceIdCharAt('y', 1)))
        )
      )
  }
  ...
  test("parses 'tag() contains' and nested 'tag() position is'") {
    runParser(
      "tag(market) contains foo and (tag(trim) position(1) is x or tag(trim) position(2) is y)"
    ) shouldBe
      And(
        NonEmptyList.of(
          TagContains("market", "foo"),
          Or(NonEmptyList.of(TagCharAt("trim", 'x', 0), TagCharAt("trim", 'y', 1)))
        )
      )
  }
  ...
  test("removes device tag from expressions") {
    ...
    Not(TagContains(land, "abc")).dropDeviceTag(country).value shouldBe Not(
      TagContains(land, "abc")
    )
    And(NonEmptyList.of(TagContains(country, "abc"), TagCharAt(country, 'a', 0)))
      .dropDeviceTag(country) shouldBe None
    And(NonEmptyList.of(TagContains(country, "abc"), DeviceIdContains("abc")))
      .dropDeviceTag(country)
      .value shouldBe DeviceIdContains("abc")
    Or(NonEmptyList.of(DeviceIdContains("abc"), TagCharAt(country, 'a', 0)))
      .dropDeviceTag(country)
      .value shouldBe DeviceIdContains("abc")
    Or(NonEmptyList.of(DeviceIdContains("abc"), DeviceIdContains("def")))
      .dropDeviceTag(country)
      .value shouldBe Or(NonEmptyList.of(DeviceIdContains("abc"), DeviceIdContains("def")))
    And(
      NonEmptyList.of(
        TagContains(country, "abc"),
        TagCharAt(country, 'a', 0),
        Or(NonEmptyList.of(DeviceIdContains("abc"), TagContains(country, "abc")))
      )
...
class GroupExpressionRunSpec extends AnyFunSuite with Matchers with DatabaseSpec with ScalaFutures {
  ...
  override implicit def patienceConfig: PatienceConfig =
    PatienceConfig(timeout = Span(3, Seconds), interval = Span(30, Millis))
  ...
  override def beforeAll(): Unit = {
    ...
    val tags = Map("market" -> "Germany", "trim" -> "Premium").map { case (k, v) =>
      stringToTagString(k) -> v
    }
    val io =
      List(create(ns, device0), create(ns, device1), tagDeviceByOemId(ns, device0.deviceId, tags))
  ...
  test("matches both expressions when using and") {
    runGroupExpression(s"(deviceid contains A) and (deviceid contains C)") shouldBe Seq(
      device0.uuid.get
    )
  ...
  test("matches all expressions when using and") {
    runGroupExpression(
      s"((deviceid contains A) and (deviceid contains B)) and (deviceid contains C)"
    ) shouldBe Seq(device0.uuid.get)
  }
  ...
  test("returns matching 'deviceid position is'") {
    runGroupExpression(s"deviceid position(1) is d") should contain theSameElementsAs Seq(
      device0.uuid.get,
      device1.uuid.get
    )
  ...
  test("'deviceid position is' and 'deviceid position is not' cancel out") {
    runGroupExpression(
      s"deviceid position(1) is d and deviceid position(1) is not d"
    ) shouldBe empty
  }

  test("'deviceid position is' or 'deviceid position is not' behaves as a tautology") {
    runGroupExpression(
      s"deviceid position(8) is B or deviceid position(8) is not B"
    ) should contain only (device0.uuid.get, device1.uuid.get)
  }
  ...
  test("returns matching 'deviceid position is not' is case-insensitive") {
    runGroupExpression(s"deviceid position(1) is not X") should contain theSameElementsAs Seq(
      device0.uuid.get,
      device1.uuid.get
    )
    runGroupExpression(s"deviceid position(8) is not b") should contain only device1.uuid.get
  ...
  test("matches all 'deviceid position is not' when position is out of bounds") {
    runGroupExpression(s"deviceid position(9) is not X") should contain theSameElementsAs Seq(
      device0.uuid.get,
      device1.uuid.get
    )
    runGroupExpression(s"deviceid position(1000) is not Y") should contain theSameElementsAs Seq(
      device0.uuid.get,
      device1.uuid.get
    )
  }
  ...
  test("returns matching 'deviceid position is' with or") {
    runGroupExpression(
      s"deviceid position(7) is E or deviceid position(8) is E"
    ) should contain only device1.uuid.get
  }

  test("returns matching 'deviceid position is' with and") {
    runGroupExpression(
      s"deviceid position(7) is D and deviceid position(8) is E"
    ) should contain only device1.uuid.get
  }

  test("returns matching 'deviceid position is not' with and") {
    runGroupExpression(
      s"deviceid position(7) is not D and deviceid position(8) is not E"
    ) should contain only device0.uuid.get
  }

  test(
    "returns matching 'deviceid contains' or 'deviceid position is' or 'deviceid position is not' when either condition is true"
  ) {
    runGroupExpression(
      s"deviceid contains evic or deviceid position(9) is Z or deviceid position(9) is not X"
    ) should contain theSameElementsAs Seq(device0.uuid.get, device1.uuid.get)
    runGroupExpression(
      s"deviceid contains nope or deviceid position(9) is C or deviceid position(9) is not F"
    ) should contain only device0.uuid.get
    runGroupExpression(
      s"deviceid contains nope or deviceid position(9) is Z or deviceid position(9) is not C"
    ) should contain only device1.uuid.get
  }

  test(
    "does not match 'deviceid contains' or 'deviceid position is' or 'deviceid position is not' when all conditions are false"
  ) {
    runGroupExpression(
      s"deviceid contains nope or deviceid position(9) is Z or deviceid position(1) is not D"
    ) shouldBe empty
  }

  test(
    "returns matching 'deviceid contains' and 'deviceid position is' and 'deviceid position is not' when all conditions are true"
  ) {
    runGroupExpression(
      s"deviceid contains evic and deviceid position(9) is C and deviceid position(8) is not C"
    ) should contain only device0.uuid.get
  }

  test(
    "does not match 'deviceid contains' and 'deviceid position is' and 'deviceid position is not' when either condition is false"
  ) {
    runGroupExpression(
      s"deviceid contains nope and deviceid position(9) is C and deviceid position(8) is not C"
    ) shouldBe empty
    runGroupExpression(
      s"deviceid contains evic and deviceid position(9) is Z and deviceid position(8) is not C"
    ) shouldBe empty
    runGroupExpression(
      s"deviceid contains evic and deviceid position(9) is C and deviceid position(8) is not B"
    ) shouldBe empty
  }
  ...
  test("matches a device by OR on two custom tags") {
    runGroupExpression(
      "tag(market) contains erma or tag(trim) position(1) is P"
    ) should contain only device0.uuid.get
    runGroupExpression(
      "tag(market) contains XermaX or tag(trim) position(1) is P"
    ) should contain only device0.uuid.get
    runGroupExpression(
      "tag(market) contains erma or tag(trim) position(1) is X"
    ) should contain only device0.uuid.get
    runGroupExpression("tag(market) contains XermaX or tag(trim) position(1) is X") shouldBe empty
    runGroupExpression(
      "tag(market) contains erma or tag(trim) position(1) is not P"
    ) should contain only (device0.uuid.get, device1.uuid.get)
    runGroupExpression(
      "tag(market) contains XermaX or tag(trim) position(1) is not P"
    ) should contain only device1.uuid.get
  }

  test("matches a device by AND on two custom tags") {
    runGroupExpression(
      "tag(market) contains erma and tag(trim) position(1) is P"
    ) should contain only device0.uuid.get
    ...
    runGroupExpression(
      "tag(market) contains erma and tag(trim) position(1) is not X"
    ) should contain only device0.uuid.get
    runGroupExpression(
      "tag(market) contains XermaX and tag(trim) position(1) is not P"
    ) shouldBe empty
  }

  test("matches device by deviceId and tagId") {
    runGroupExpression(
      "tag(market) position(1) is G and deviceid contains abc"
    ) should contain only device0.uuid.get
    runGroupExpression(
      "tag(market) contains erma and deviceid contains abc"
    ) should contain only device0.uuid.get
    runGroupExpression(
      "tag(market) position(1) is not X and deviceid contains abc"
    ) should contain only device0.uuid.get
    runGroupExpression(
      "deviceid position(1) is d and tag(market) position(1) is G"
    ) should contain only device0.uuid.get
    runGroupExpression(
      "deviceid position(1) is d and tag(market) position(1) is not X"
    ) should contain only (device0.uuid.get, device1.uuid.get)
    runGroupExpression(
      "deviceid position(1) is d and tag(market) contains erma"
    ) should contain only device0.uuid.get
  }
