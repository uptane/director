...
class FakeKeyserverClient extends KeyserverClient {
  ...
  override def addOfflineUpdatesRole(repoId: RepoId): Future[Unit] = async {
    ...
    val newRoles = rootRole.roles ++ Map(RoleType.OFFLINE_UPDATES -> roleKeys, RoleType.OFFLINE_SNAPSHOT -> roleKeys)
  ...
  override def addRemoteSessionsRole(repoId: RepoId): Future[Unit] =
    throw new NotImplementedError("not implemented for FakeKeyserverClient")
}
