...
class ManifestCompilerSpec extends DirectorSpec {
  ...
  test("manifest setting already known versions is a NOOP") {
    ...
    val currentStatus = DeviceKnownState(deviceId, primary, Map(primary -> Some(ecuTarget.id), secondary -> Some(ecuTarget.id)), Map(ecuTarget.id -> ecuTarget), Set.empty, Set.empty, Set.empty, Map.empty, generatedMetadataOutdated = false)
  ...
  test("manifest setting unknown ecu targets creates targets") {
    ...
    val currentStatus = DeviceKnownState(deviceId, primary, Map.empty, Map.empty, Set.empty, Set.empty, Set.empty, Map.empty, generatedMetadataOutdated = false)
  ...
  test("secondary assignment is completed if target is installed") {
    ...
    val currentStatus = DeviceKnownState(deviceId, primary, Map(primary -> None, secondary -> None), Map(ecuTarget.id -> ecuTarget), Set(assignment, secondaryAssignment), Set.empty, Set.empty, Map.empty, generatedMetadataOutdated = false)

    val newStatus = ManifestCompiler(ns, manifest).apply(currentStatus).get.knownState

    newStatus.currentAssignments shouldBe Set(assignment)
    newStatus.processedAssignments.map(_.correlationId) shouldBe Set(secondaryAssignment.correlationId)
  ...
  test("assignment is completed if target is installed") {
    ...
    val currentStatus = DeviceKnownState(deviceId, primary, Map(primary -> None), Map(ecuTarget.id -> ecuTarget), Set(assignment, otherAssignment), Set.empty, Set.empty, Map.empty, generatedMetadataOutdated = false)
  ...
  test("assignment is completed if target was not installed and report is not successful") {
    ...
    val currentStatus = DeviceKnownState(deviceId, primary, Map(primary -> None), Map(ecuTarget.id -> ecuTarget), Set(assignment), Set.empty, Set.empty, Map.empty, generatedMetadataOutdated = false)
  ...
  test("scheduled assignment is completed if all scheduled updates are installed without installation report") {
    val ecuManifest = EcuManifest(targetUpdate.toImage, primary, "")
    val ecuVersionManifest = Map(primary -> SignedPayload(Seq.empty, ecuManifest, ecuManifest.asJson))

    val manifest = DeviceManifest(primary, ecuVersionManifest, Left(MissingInstallationReport))

    val scheduledEcuTarget = EcuTarget(ns, EcuTargetId.generate(), targetUpdate.target, targetUpdate.targetLength, targetUpdate.checksum, targetUpdate.checksum.hash, targetUpdate.uri, targetUpdate.userDefinedCustom)

    val su = ScheduledUpdate(ns, ScheduledUpdateId.generate(), deviceId, UpdateId.generate(), Instant.now(), ScheduledUpdate.Status.Assigned)
    ...
    val currentStatus = DeviceKnownState(deviceId, primary, Map(primary -> None), ecuTargets, Set(assignment), Set.empty, Set(su), ecuTargetsByHardwareId, generatedMetadataOutdated = false)
  ...
  test("scheduled assignment is partially completed if some scheduled updates are installed without installation report") {
    val ecuManifest = EcuManifest(targetUpdate.toImage, primary, "")
    val ecuVersionManifest = Map(primary -> SignedPayload(Seq.empty, ecuManifest, ecuManifest.asJson))
    ...
    val scheduledEcuTarget1 = EcuTarget(ns, EcuTargetId.generate(), targetUpdate1.target, targetUpdate1.targetLength, targetUpdate1.checksum, targetUpdate1.checksum.hash, targetUpdate1.uri, targetUpdate1.userDefinedCustom)

    val su = ScheduledUpdate(ns, ScheduledUpdateId.generate(), deviceId, UpdateId.generate(), Instant.now(), ScheduledUpdate.Status.Assigned)

    val ecuTargets = Map(ecuTarget.id -> ecuTarget, scheduledEcuTarget.id -> scheduledEcuTarget, scheduledEcuTarget1.id -> scheduledEcuTarget1)

    val ecuTargetsByHardwareId = Map(su.updateId -> List(scheduledEcuTarget.id, scheduledEcuTarget1.id))
  ...
  test("scheduled assignment is completed if update is installed without installation report, when the scheduled update does not have an assignment yet") {
    val ecuManifest = EcuManifest(targetUpdate.toImage, primary, "")
    val ecuVersionManifest = Map(primary -> SignedPayload(Seq.empty, ecuManifest, ecuManifest.asJson))
    ...
    val su = ScheduledUpdate(ns,
      ScheduledUpdateId.generate(), deviceId, UpdateId.generate(), Instant.now(), ScheduledUpdate.Status.Assigned)

    val ecuTargets = Map(ecuTarget.id -> ecuTarget, scheduledEcuTarget.id -> scheduledEcuTarget)

    val ecuTargetsByHardwareId = Map(su.updateId -> List(scheduledEcuTarget.id))

    val currentStatus = DeviceKnownState(deviceId, primary, Map(primary -> None), ecuTargets, Set(), Set.empty, Set(su), ecuTargetsByHardwareId, generatedMetadataOutdated = false)
